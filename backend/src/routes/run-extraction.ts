...
function classifyDoc(fileName: string): DocBucket {
  ...
  if (/\bpip\b/.test(n))                                                       return "capex";
  if (/\bstr\b/.test(n))                                                       return "market";
...
function safeParseHelper(s: string): string {
  if (!s) return "{}";
...
async function getDocumentText(
  ...
  try {
    const result = await extractDocumentText(buffer, doc.file_name, { nvidiaApiKey: process.env.NVIDIA_API_KEY, pdfScale: 1.5 });
...
export async function runExtractionHandler(req: Request, res: Response) {
  ...
  try {
    ...
    if (existing?.length) {
      await db.from("ai_jobs").update({ status: "running" }).eq("id", (existing[0] as { id: string }).id);
    } else {
    ...
    const distillOne = async (d: { id: string; name: string; text: string; bucket: DocBucket }) => {
      if (d.text.length < 200) return { ...d, facts: d.text };
      ...
      for (const chunk of chunks) {
        ...
        } catch (err) { console.error(`[distill] failed for ${d.name}:`, err); }
      }
    ...
    const BUDGET = 5000;
    const ESTIMATES: Record<string, number> = { metadata: 12, financial: 20, unit_mix: 12, summary: 12, questions: 8, criteria: 10, risks: 12 };
    ...
    const runAgent = async (name: string, fn: () => Promise<{ ok?: boolean; rawPreview?: string; summary?: string }>) => {
      ...
      try {
        ...
        if (result?.summary) emit(res, { type: "log", agent: name, message: `✓ ${result.summary} · ${dt.toFixed(1)}s (${vStr})` });
        else                 emit(res, { type: "log", agent: name, message: `✓ ${name} done · ${dt.toFixed(1)}s (${vStr})` });
...
${ctx}`,
      ...
      if (!metaJson.city || !metaJson.state) {
        ...
        if (m) { if (!metaJson.city) metaJson.city = m[1].trim(); if (!metaJson.state) metaJson.state = m[2]; }
      }
      ...
      if (!metaJson.units) {
        ...
        for (const m of matches) {
          ...
          if (n >= 40 && n <= 2000 && !(n >= 1900 && n <= 2100)) { metaJson.units = n; break; }
        }
      }
    ...
    await Promise.all([
      ...
      runAgent("risks", async () => {
        ...
        if (j.explanations?.length) { await db.from("ai_explanations").delete().eq("deal_id", dealId); await db.from("ai_explanations").insert(j.explanations.map(e => ({ deal_id: dealId, field_name: e.fieldName, explanation_text: e.explanationText, source_document_id: parsedDocs[0]?.id||null, source_page: e.sourcePage||null, source_snippet: e.sourceSnippet||null }))); }
