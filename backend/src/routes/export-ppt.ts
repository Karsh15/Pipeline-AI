...
export async function exportPptHandler(req: Request, res: Response) {
  ...
  metrics.forEach((m, i) => {
    ...
    const x = 0.3 + col * 3.2; const y = 1.1 + row * 1.5;
  ...
  if (unitMix?.length) {
    ...
    ["Unit Type","Total Units","Vacant","Occ %","Avg Rent"].forEach((h, i) =>
      s4.addText(h, { x: 0.3 + i * 1.85, y: 1.0, w: 1.8, h: 0.4, fontSize: 9, bold: true, color: WHITE, align: "center", fill: { color: ORANGE } })
    );
    unitMix.slice(0, 8).forEach((u, ri) => {
      const y = 1.5 + ri * 0.48;
      const occ = u.total_units > 0 ? (((u.total_units - u.vacant_units) / u.total_units)*100).toFixed(0) + "%" : "—";
      [u.unit_type, String(u.total_units), String(u.vacant_units), occ, `$${u.avg_rent.toLocaleString()}`].forEach((val, ci) =>
        s4.addText(val, { x: 0.3 + ci * 1.85, y, w: 1.8, h: 0.42, fontSize: 10, align: "center", color: DARK, fill: { color: ri % 2 === 0 ? "F8FAFC" : WHITE } })
      );
    });
  }
  ...
  if (financials?.length) {
    ...
    ["Metric","2021","2022","2023","TTM"].forEach((h, i) =>
      s5.addText(h, { x: 0.3 + i * 1.85, y: 1.0, w: 1.8, h: 0.4, fontSize: 9, bold: true, color: WHITE, align: "center", fill: { color: ORANGE } })
    );
    const fmt = (n: number) => n ? `$${(n/1000).toFixed(0)}K` : "—";
    financials.slice(0, 8).forEach((f, ri) => {
      const y = 1.5 + ri * 0.48;
      [f.sub_category, fmt(f.y2021), fmt(f.y2022), fmt(f.y2023), fmt(f.ttm)].forEach((val, ci) =>
        s5.addText(val, { x: 0.3 + ci * 1.85, y, w: 1.8, h: 0.42, fontSize: 10, align: ci === 0 ? "left" : "center", color: ci === 4 ? ORANGE : DARK, bold: ci === 4, fill: { color: ri % 2 === 0 ? "F8FAFC" : WHITE } })
      );
    });
  }
