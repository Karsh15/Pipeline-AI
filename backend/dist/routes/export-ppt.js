...
exports.exportPptHandler = exportPptHandler;
...
const DARK = "1E293B";
const GRAY = "64748B";
async function exportPptHandler(req, res) {
    const { dealId } = req.body;
    const db = (0, supabase_1.supabaseAdmin)();
    const [{ data: deal }, { data: financials }, { data: unitMix }, { data: risks }, { data: criteria }, { data: extracted },] = await Promise.all([
    ...
    const getField = (name) => extracted?.find(e => e.field_name === name)?.value || "";
    // ── Slide 1: Cover ────────────────────────────────────────────────────────
    const s1 = pptx.addSlide();
    s1.background = { color: DARK };
    s1.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: 0.5, h: 5.63, fill: { color: ORANGE } });
    s1.addText(deal?.name || "Deal Analysis", { x: 0.8, y: 1.5, w: 8.5, h: 1.2, fontSize: 36, bold: true, color: WHITE, fontFace: "Calibri" });
    s1.addText(`${deal?.address || ""} · ${deal?.city || ""}, ${deal?.state || ""}`, { x: 0.8, y: 2.8, w: 8.5, h: 0.5, fontSize: 14, color: "AAAAAA" });
    s1.addText(`${(deal?.asset_type || "").toUpperCase()} · ${deal?.units || 0} UNITS · ${new Date().getFullYear()}`, { x: 0.8, y: 3.4, w: 8.5, h: 0.4, fontSize: 11, color: ORANGE, bold: true });
    s1.addText("CONFIDENTIAL — AI-GENERATED UNDERWRITING ANALYSIS", { x: 0.8, y: 4.8, w: 8.5, h: 0.3, fontSize: 8, color: "888888" });
    ...
    s3.addText(`${deal?.city || ""}, ${deal?.state || ""}`, { x: 0.4, y: 1.0, w: 9, h: 0.6, fontSize: 24, bold: true, color: DARK });
    s3.addText(deal?.location_insight || "Location analysis pending.", { x: 0.4, y: 1.8, w: 9, h: 1.5, fontSize: 13, color: GRAY });
    s3.addText(deal?.broker_narrative || "Broker narrative pending.", { x: 0.4, y: 3.5, w: 9, h: 1.5, fontSize: 11, color: "94A3B8" });
    // ── Slide 4: Unit Mix ─────────────────────────────────────────────────────
    if (unitMix?.length) {
        ...
        ["Unit Type", "Total Units", "Vacant", "Occ %", "Avg Rent"].forEach((h, i) => s4.addText(h, { x: 0.3 + i * 1.85, y: 1.0, w: 1.8, h: 0.4, fontSize: 9, bold: true, color: WHITE, align: "center", fill: { color: ORANGE } }));
        unitMix.slice(0, 8).forEach((u, ri) => {
            const y = 1.5 + ri * 0.48;
            const occ = u.total_units > 0 ? (((u.total_units - u.vacant_units) / u.total_units) * 100).toFixed(0) + "%" : "—";
            [u.unit_type, String(u.total_units), String(u.vacant_units), occ, `$${u.avg_rent.toLocaleString()}`].forEach((val, ci) => s4.addText(val, { x: 0.3 + ci * 1.85, y, w: 1.8, h: 0.42, fontSize: 10, align: "center", color: DARK, fill: { color: ri % 2 === 0 ? "F8FAFC" : WHITE } }));
        });
    }
    // ── Slide 5: Financials ───────────────────────────────────────────────────
    if (financials?.length) {
        ...
        ["Metric", "2021", "2022", "2023", "TTM"].forEach((h, i) => s5.addText(h, { x: 0.3 + i * 1.85, y: 1.0, w: 1.8, h: 0.4, fontSize: 9, bold: true, color: WHITE, align: "center", fill: { color: ORANGE } }));
        const fmt = (n) => n ? `$${(n / 1000).toFixed(0)}K` : "—";
        financials.slice(0, 8).forEach((f, ri) => {
            const y = 1.5 + ri * 0.48;
            [f.sub_category, fmt(f.y2021), fmt(f.y2022), fmt(f.y2023), fmt(f.ttm)].forEach((val, ci) => s5.addText(val, { x: 0.3 + ci * 1.85, y, w: 1.8, h: 0.42, fontSize: 10, align: ci === 0 ? "left" : "center", color: ci === 4 ? ORANGE : DARK, bold: ci === 4, fill: { color: ri % 2 === 0 ? "F8FAFC" : WHITE } }));
        });
    }
    // ── Slide 6: Risks ────────────────────────────────────────────────────────
    ...
    (risks || []).slice(0, 6).forEach((r, i) => {
        ...
        s6.addText(`[${r.severity.toUpperCase()}] ${r.description}`, { x: 0.55, y: y + 0.1, w: 9.0, h: 0.4, fontSize: 10, color: DARK });
    });
    ...
    s7.addText(getField("rationale") || "Analysis pending final review.", { x: 0.6, y: 2.9, w: 8.8, h: 1.2, fontSize: 13, color: GRAY, align: "center" });
    const criteriaPass = (criteria || []).filter(c => c.meets).length;
    s7.addText(`Buy Box: ${criteriaPass}/${(criteria || []).length} criteria met · Buy Box Score: ${getField("buyBoxScore") || "—"}/100`, { x: 0.6, y: 4.3, w: 8.8, h: 0.4, fontSize: 11, color: ORANGE, align: "center", bold: true });
    s7.addText("Generated by Antigravity CRE Underwriting OS", { x: 0, y: 5.1, w: 10, h: 0.3, fontSize: 8, color: "CCCCCC", align: "center" });
    const buffer = await pptx.write({ outputType: "nodebuffer" });
    const filename = `${(deal?.name || "deal").replace(/\s+/g, "_")}_analysis.pptx`;
