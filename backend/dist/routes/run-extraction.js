        ...
        await runAgent("financial", async () => {
            ...
            const parseDataLine = (line) => {
                ...
                if (nums.length >= 2 && nums.length <= 6) {
                    ...
                    nums.forEach((n, i) => { if (i < yearSlots.length)
                        result[yearSlots[i + (5 - nums.length)]] = n.v; });
            ...
            for (const line of allText.split("\n")) {
                ...
                if (!nums || !Object.values(nums).some(v => v !== 0))
                    continue;
            ...
            const kpi = (pattern, src = fullText) => {
            ...
            if (regexRows.length < 10 || missingCritical) {
                ...
                try {
                    const raw = await (0, llm_1.chat)({
                        agent: "financial", model: llm_1.MODELS.STANDARD, max_tokens: 1200, temperature: 0.0,
                        messages: [{
                                role: "system",
...
${ctxSlice}`,
                    ...
                    const parsed = (0, llm_1.safeParse)(raw, {});
                    llmRows = parsed.financials || [];
                    // Supplement KPIs from LLM if regex missed them
                    const lk = parsed.kpis || {};
                    if (!kpiOccupancy && lk.occupancy)
                        Object.assign(lk, { _occ: lk.occupancy });
                    if (!kpiADR && lk.adr)
                        Object.assign(lk, { _adr: lk.adr });
                }
                catch (err) {
                    console.error("[financial] LLM gap-fill failed:", err);
                }
            }
            ...
            if (kpiDSCR)
                dealUpd.dscr = kpiDSCR;
            if (kpiOccupancy)
                dealUpd.occupancy_rate = kpiOccupancy;
            if (!kpiCapRate && noiVal) {
                ...
                if (gp && gp > 0)
                    dealUpd.cap_rate = parseFloat(((noiVal / gp) * 100).toFixed(2));
            }
            ...
            const kpiRows = [];
            if (kpiADR && !mergedMap.has("adr"))
                kpiRows.push({ deal_id: dealId, category: "income", sub_category: "ADR", y2021: 0, y2022: 0, y2023: 0, y2024: 0, y2025: 0, ttm: kpiADR, m1: 0, m2: 0, m3: 0, m4: 0, m5: 0, m6: 0, m7: 0, m8: 0, m9: 0, m10: 0, m11: 0, m12: 0, per_unit: 0, pct_egi: 0 });
            if (kpiRevPAR && !mergedMap.has("revpar"))
                kpiRows.push({ deal_id: dealId, category: "income", sub_category: "RevPAR", y2021: 0, y2022: 0, y2023: 0, y2024: 0, y2025: 0, ttm: kpiRevPAR, m1: 0, m2: 0, m3: 0, m4: 0, m5: 0, m6: 0, m7: 0, m8: 0, m9: 0, m10: 0, m11: 0, m12: 0, per_unit: 0, pct_egi: 0 });
            if (kpiExpRatio)
                kpiRows.push({ deal_id: dealId, category: "expense", sub_category: "Expense Ratio %", y2021: 0, y2022: 0, y2023: 0, y2024: 0, y2025: 0, ttm: kpiExpRatio, m1: 0, m2: 0, m3: 0, m4: 0, m5: 0, m6: 0, m7: 0, m8: 0, m9: 0, m10: 0, m11: 0, m12: 0, per_unit: 0, pct_egi: 0 });
            const allRows = [...rows, ...kpiRows];
            await db.from("financials").delete().eq("deal_id", dealId);
            if (allRows.length)
                await db.from("financials").insert(allRows);
