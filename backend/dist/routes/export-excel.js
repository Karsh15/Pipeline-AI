...
function currency(ws, col, value) {
    const cell = ws.getCell(col);
    cell.value = value ?? 0;
    cell.numFmt = "$#,##0";
...
async function exportExcelHandler(req, res) {
    ...
    ["A1", "B1", "C1", "D1", "E1"].forEach((col, i) => header(ws2, col, ["METRIC", "2021", "2022", "2023", "TTM"][i]));
    ...
    ["A1", "B1", "C1", "D1", "E1"].forEach((col, i) => header(ws3, col, ["UNIT TYPE", "TOTAL UNITS", "VACANT", "OCC %", "AVG RENT"][i]));
    (unitMix || []).forEach((u, i) => {
        const rn = i + 2;
    ...
    const colorMap = { critical: "FFEF4444", high: "FFF97316", medium: "FFF59E0B", low: "FF10B981" };
    (risks || []).forEach((r, i) => {
        const rn = i + 2;
    ...
    ["A1", "B1", "C1", "D1"].forEach((col, i) => header(ws6, col, ["CRITERIA", "REQUIREMENT", "ACTUAL", "MEETS"][i]));
