...
exports.outlookStatusHandler = outlookStatusHandler;
const supabase_1 = require("../lib/supabase");
async function outlookStatusHandler(_req, res) {
    ...
    const tokenExpired = tokenRow ? new Date(tokenRow.expires_at) < new Date() : false;
    const { data: ingestions } = await db
        .from("email_ingestions")
        .select("id, outlook_message_id, deal_id, subject, sender_name, sender_email, received_at, attachment_count, status, error_message, created_at")
