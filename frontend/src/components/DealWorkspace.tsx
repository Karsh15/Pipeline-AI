        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-border">
              ...
              <th className="px-2 py-2 text-left text-[9px] font-black text-primary uppercase tracking-widest w-[120px] min-w-[120px] max-w-[120px] bg-[#FFF6ED]">
                <div className="font-black leading-tight line-clamp-2 break-words normal-case text-[10px]">{deal.name}</div>
                {(deal.city || deal.state) && (
                  <div className="text-[9px] font-normal text-muted-foreground normal-case mt-0.5 truncate">{[deal.city, deal.state].filter(Boolean).join(", ")}</div>
                )}
                <div className="text-[9px] font-normal text-muted-foreground normal-case">(This Deal)</div>
              </th>
              {comps.map(({ deal: d, score: s }) => (
                <th key={d.id} className="px-2 py-2 text-left text-[9px] font-black text-muted-foreground uppercase tracking-widest w-[120px] min-w-[120px] max-w-[120px]">
                  <div className="font-black leading-tight line-clamp-2 break-words normal-case text-[10px]">{d.name}</div>
                  {(d.city || d.state) && (
                    <div className="text-[9px] font-normal normal-case text-slate-500 mt-0.5 truncate">{[d.city, d.state].filter(Boolean).join(", ")}</div>
                  )}
