...
import {
  ...
  Building2, MapPin, DollarSign, BarChart3, Users, Calendar,
} from "lucide-react";
...
const STATUS_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  ingestion:    { bg: "bg-sky-50",     text: "text-sky-700"     },
  extraction:   { bg: "bg-indigo-50",  text: "text-indigo-700"  },
  underwriting: { bg: "bg-violet-50",  text: "text-violet-700"  },
  review:       { bg: "bg-emerald-50", text: "text-emerald-700" },
  completed:    { bg: "bg-green-50",   text: "text-green-700"   },
  Tracking:     { bg: "bg-blue-50",    text: "text-blue-700"    },
  Underwriting: { bg: "bg-purple-50",  text: "text-purple-700"  },
  LOI:          { bg: "bg-amber-50",   text: "text-amber-700"   },
  Dead:         { bg: "bg-red-50",     text: "text-red-600"     },
};
...
export default function PipelinePage() {
  ...
  const avgCapRate    = capRateDeals.length
    ? capRateDeals.reduce((s, d) => s + Number(d.cap_rate), 0) / capRateDeals.length
    : 0;
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {loading
                  ...
                  : filtered.map((d, i) => {
                      const sc = STATUS_COLORS[d.status] ?? { bg: "bg-slate-100", text: "text-slate-600" };
                      return (
                        <motion.tr key={d.id}
                          ...
                          <td className="px-4 py-3 font-semibold text-foreground">
                            <div className="max-w-[220px] truncate">{d.name}</div>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${sc.bg} ${sc.text}`}>
                              {d.status}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1 text-muted-foreground">
                              <Users className="h-3 w-3" />{d.units ?? "—"}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1 text-muted-foreground">
                              <Calendar className="h-3 w-3" />{d.year_built ?? "—"}
                            </div>
                          </td>
                          <td className="px-4 py-3 text-muted-foreground">
                            <div className="max-w-[130px] truncate">{d.broker || "—"}</div>
                          </td>
                          <td className="px-4 py-3 text-muted-foreground">{d.brand || "—"}</td>
                          <td className="px-4 py-3 text-muted-foreground">
                            <div className="max-w-[120px] truncate">{d.deal_lead || "—"}</div>
                          </td>
                          <td className="px-4 py-3 font-semibold text-foreground">{fmtMoney(d.guidance_price)}</td>
                          <td className="px-4 py-3">
                            {d.noi && d.noi > 0
                              ? <span className="flex items-center gap-1 font-semibold text-emerald-600"><TrendingUp className="h-3 w-3" />{fmtMoney(d.noi)}</span>
                              : <span className="text-muted-foreground">—</span>}
                          </td>
                          <td className="px-4 py-3">
                            {d.cap_rate && Number(d.cap_rate) > 0
                              ? <span className="font-semibold text-violet-600">{d.cap_rate}%</span>
                              : <span className="text-muted-foreground">—</span>}
                          </td>
                        </motion.tr>
                      );
          ...
          {!loading && (
            <div className="px-6 py-3 border-t border-border bg-slate-50 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">
                Showing <span className="font-bold text-foreground">{filtered.length}</span> of {deals.length} properties
              </span>
