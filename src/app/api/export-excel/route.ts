...
export async function POST(req: NextRequest) {
  ...
  const uwFields = extracted?.filter(e =>
    ["underwrittenNOI","stabilizedCapRate","dscr","cashOnCash","irr5yr","buyBoxScore","recommendation","rationale"].includes(e.field_name)
  ) || [];

  uwFields.forEach((f, i) => {
    const rn = i + 2;
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${(deal?.name || "deal").replace(/\s+/g,"_")}_underwriting.xlsx"`,
    },
  });
}
