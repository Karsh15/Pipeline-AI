...
export async function POST(req: NextRequest) {
  const { dealId, message, history } = await req.json() as {
    dealId: string;
    message: string;
  ...
  // Gather deal context from DB
  const [{ data: deal }, { data: financials }, { data: risks }, { data: questions }, { data: unitMix }, { data: criteria }] =
    await Promise.all([
  ...
  const messages = [
    {
      ...
      content: `You are an expert CRE (Commercial Real Estate) investment analyst AI.
You have full access to the deal data below. Answer questions precisely, cite specific numbers, and provide actionable insights.
Always reference the actual data when available. Format financial figures clearly.
...
${context}`,
  ...
  // Find relevant source citations from explanations
