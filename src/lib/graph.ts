...
async function fetchMe(accessToken: string): Promise<{ mail: string; userPrincipalName: string }> {
...
export async function fetchDealEmails(
  accessToken: string,
  since: Date
  ...
  const data = await graphGet<{ value: GraphMessage[] }>(accessToken, "/me/messages", {
    $filter:  filter,
...
export function attachmentToBuffer(attachment: GraphAttachment): Buffer {
